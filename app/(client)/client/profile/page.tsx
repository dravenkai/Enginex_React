import Image from "next/image";
import { Camera, Mail, MapPin } from "lucide-react";

export default function ProfilePage() {
  return (
    <div className="p-8 space-y-8 max-w-[1400px] mx-auto">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-stretch">
        <div className="lg:col-span-2 bg-white border-4 border-black p-8 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] flex flex-col sm:flex-row gap-6">
          <div className="relative w-28 h-28 shrink-0 border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] overflow-hidden">
            <Image src="/profile.avif" alt="Khant Nyar Thwin" fill className="object-cover" />
            
          </div>
          <button
              type="button"
              aria-label="Change profile photo"
              className="absolute top-59 left-108 shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] w-8 h-8 grid place-items-center bg-[#93c5fd] border-2 border-black"
            >
              <Camera className="w-4 h-4" />
          </button>

          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-4">
              <h2 className="text-2xl font-black">KHANT NYAR THWIN</h2>
              <button
                type="button"
                className="shrink-0 border-2 border-black bg-[#fef08a] hover:bg-[#f5e35a] px-4 py-2 text-xs font-bold uppercase shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] active:translate-x-1 active:translate-y-1 active:shadow-none transition-all"
              >
                Edit Profile
              </button>
            </div>

            <div className="mt-2 space-y-1 text-xs text-gray-600 pb-3 border-b border-black">
              <div className="flex items-center gap-1.5">
                <MapPin className="w-3.5 h-3.5" />
                Mandalay Township
              </div>
              <div className="flex items-center gap-1.5">
                <Mail className="w-3.5 h-3.5" />
                m.thorne@horizonsystems.io
              </div>
            </div>

            <p className="mt-3 text-sm text-gray-700 leading-6">
              Senior Infrastructure Director focused on scaling distributed cloud architectures.
              Dedicated to building reliable, high-performance systems for global enterprises.
            </p>
          </div>
        </div>

        <div className="bg-[#7ca3ef] border-4 border-black p-8 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] flex flex-col items-center justify-center text-center text-white">
          <span className="text-xs font-bold uppercase tracking-widest">Active Projects</span>
          <span className="text-6xl font-black my-3">12</span>
          <button
            type="button"
            className="w-full border-2 border-black bg-white text-black px-4 py-3 text-xs font-bold uppercase shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] active:translate-x-1 active:translate-y-1 active:shadow-none transition-all"
          >
            View All Projects
          </button>
        </div>
      </div>

      <div className="bg-gray-100 border-4 border-black p-8 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] flex flex-col md:flex-row md:items-center md:justify-between gap-6">
        <div>
          <h2 className="text-lg font-bold">ACCOUNT SECURITY</h2>
          <p className="mt-2 max-w-lg text-sm text-gray-600 leading-6">
            Your profile is currently protected by Two-Factor Authentication. Keep your contact
            information updated to ensure project continuity.
          </p>
        </div>

        <div className="flex flex-col gap-3 shrink-0 sm:w-56">
          <button
            type="button"
            className="border-2 border-black bg-white px-4 py-3 text-xs font-bold uppercase shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] active:translate-x-1 active:translate-y-1 active:shadow-none transition-all"
          >
            Security Log
          </button>
          <button
            type="button"
            className="border-2 border-black bg-[#fef08a] hover:bg-[#f5e35a] ml-2 px-4 py-3 text-xs font-bold uppercase shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] active:translate-x-1 active:translate-y-1 active:shadow-none transition-all"
          >
            Manage Keys
          </button>
        </div>
      </div>
    </div>
  );
}
